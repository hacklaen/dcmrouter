 ...
 * Copyright (c) 2003 by
 *
 * IFTM Institut für Telematik in der Medizn GmbH VISUS Technology Transfer GmbH
 *
...
import de.iftm.dcm4che.router.util.DicomRouterEventListener;
import de.iftm.dcm4che.router.util.Util;
 ...
 * <p>
 * Implementations of this class should send the following DicomRouterEvent's to registered
 * listeners:
 * <p>
 * STARTING_EVENT This event should be send before processing the first DICOM object.
 * <p>
 * NEXT_OBJECT_EVENT This event should be send for each DICOM object before processing the object in
 * the plugin chain.
 * <p>
 * PROGRESS_EVENT If the total number of objects to process is known, this event should be send for
 * each DICOM object before processing the object in the plugin chain.
 * <p>
 * FINISHED_EVENT This event should be send after processing the last DICOM object.
 *
 * @author hacklaender
 * @version 2006.11.10
 */
public abstract class DicomReceiver {
   ...
   * <p>
   * The properties must specify at least one plugin. The definition
   * <p>
   * key = plugin.1.name, value = de.iftm.dcm4che.router.plugin.NullPlugin
   * <p>
   * is sufficient.
   *
   ...
   * Constructor of a DicomReceiver.
   * <p>
   * The referenced property file must specify at least one plugin. The definition
   * <p>
   * plugin.1.name = de.iftm.dcm4che.router.plugin.NullPlugin
   * <p>
   * is sufficient.
   *
  ...
  protected void fireDicomRouterEvent(DicomRouterEvent evt) {
    ...
    if (dicomRouterEventListeners == null)
      return;
 ...
 * Revisions:
 *
 * 2006.04.29, hacklaender: Totally rewritten. 2005.08.22, hacklaender: method execute renamed to
 * start (for consistance with a future implementation of a stop method). 2005.08.21, hacklaender:
 * initProperties method added.
 */
